...
router.get('/status', authenticateToken, (req, res) => {
  exec('pm2 jlist', (error, stdout, stderr) => {
    ...
    try {
      ...
      const botProcess = processes.find(p => p.name === 'zabbix-whatsapp-bot');
...
router.post('/stop', authenticateToken, requireAdmin, (req, res) => {
  exec('pm2 stop zabbix-whatsapp-bot', (error, stdout, stderr) => {
    if (error) {
      return res.json({
        success: false,
...
router.post('/restart', authenticateToken, requireAdmin, (req, res) => {
  exec('pm2 restart zabbix-whatsapp-bot', (error, stdout, stderr) => {
    if (error) {
      return res.json({
        success: false,
...
router.get('/logs', authenticateToken, (req, res) => {
  ...
  exec(`pm2 logs zabbix-whatsapp-bot --lines ${lines} --nostream`, (error, stdout, stderr) => {
    if (error) {
      return res.json({
        success: false,
...
router.get('/health', authenticateToken, (req, res) => {
  exec('curl -f http://localhost:3000/health', (error, stdout, stderr) => {
    if (error) {
      return res.json({
        success: false,
        message: 'Bot não está respondendo',
        healthy: false
      });
    }
    ...
    try {
      ...
      res.json({
        success: false,
        message: 'Resposta inválida do health check',
        healthy: false
      });
    }
  });
    ...
    (err, stats) => {
      ...
      db.get(
        `SELECT 
          ...
          SUM(zabbix_alerts) as total_alerts
         FROM bot_stats`,
        (err, totals) => {
          db.close();
